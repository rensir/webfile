...
function maintop($title,$showtop = true) {
    ...
    if ($showtop) {//头部菜单内容
        echo "<div class=\"menu\">\n"
            ."<li><a href=\"".$meurl."?op=home\">主页</a></li>\n"
            .$back
            ...
            ."<li><a href=\"".$meurl."?op=killme&dename=".$me."&folder=./\" onclick=\"return confirm('你确定要自杀吗？');\">自杀</a></li>\n"
...
function login($er=false) {
    ...
    if ($er) { 
        ...
        echo "<span class=error>ERROR: 用户名或密码错误！</span><br>\n"; 
    }
...
function cr() {
    ...
    echo "<form action=\"".$meurl."?op=create\" method=\"post\">\n"
        ."<label for=\"nfname\">文件名：</label><br><input type=\"text\" size=\"20\" id=\"nfname\" name=\"nfname\" class=\"text\"><br>\n"
        ."<label for=\"ndir\">目标目录：</label><br><input type=\"text\" class=\"text\" id=\"ndir\" name=\"ndir\" value=\"".$_SESSION['folder']."\">";
    echo $content
        ."<br>";

    echo "<select name=\"isfolder\"><option value=\"1\" checked>文件夹</option>\n"
        ."<option value=\"0\" checked>文件</option></select><br>\n"
        ."<input type=\"hidden\" name=\"folder\" value=\"$folder\">\n"
...
if(@$_POST['action']=='移动'){
    if(isset($_POST['select_item'])){
        ...
        echo "<input type=\"submit\" value=\"移动\" border=\"0\" class=\"button\"> <a href=\"".$meurl."?op=home&folder=".$_SESSION['folder']."\">返回文件管理</a>\n";
        mainbottom();
    }else{
        printerror("您没有选择文件");
    }
...
if(@$_POST['action']=='复制'){
    if(isset($_POST['select_item'])){
    	...
    	echo '<input type="hidden" name="file" value="'.$file.'"><input type="hidden" name="folder" value="'.$_SESSION['folder'].'">您将把下列文件复制到：'
    	    ."<input type=\"text\" class=\"text\" name=\"ndir\" value=\"".$_SESSION['folder']."\">\n"
    	    ."<div class='tips'>".$file."</div>";
        echo "<input type=\"submit\" value=\"复制\" border=\"0\" class=\"button\"> <a href=\"".$meurl."?op=home&folder=".$_SESSION['folder']."\">返回文件管理</a>\n";
        mainbottom();
    }else{
        printerror("您没有选择文件");
    }
